    ...
    public partial class UserControl2 : UserControl
    {
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                attri[2] = "ContactNumber";
                para[2] = textBox6.Text;
                for (int i = 0; i < 3; i++)
                {
                    if (para[i] != "" && !ContainsDigit(para[i]) || para[i] == "ContactNumber")
                    {
                        //para[i] = "'" + para[i] + "'";
                        string sql1 = "UPDATE ElectionOfficials SET " + attri[i] + " = :name where OfficialId = :id";
                        ...
                        if (row > 0)
                        {
                            MessageBox.Show("Official is Updated");
                        }
                        con.Close();
                    }
                    else if (ContainsDigit(para[i]))
                    {
                        string sql2 = "Update ElectionOfficials set " + attri[i] + " = :name  where OfficialId = :id;";
