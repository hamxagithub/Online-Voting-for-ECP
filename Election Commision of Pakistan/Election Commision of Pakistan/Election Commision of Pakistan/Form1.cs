...
namespace Election_Commision_of_Pakistan
{
    ...
    public static class ControlHelper
    {
        public static void SetFontForAllControls(Control.ControlCollection controls, Font font)
        {
            ...
            {
                int currentY = 10; // Initial Y position for controls


                control.Font = new Font("Monotype Corsiva", 9, FontStyle.Bold); ;

                if (control is Panel || control is GroupBox)
                {
                  // Bring the new layout panel to the front

                    control.BackColor = Color.FromArgb(100, 0, 0, 0);
                }
                if(control is System.Windows.Forms.Label|| control is RadioButton)
                { 
